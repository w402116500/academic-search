...
async def list_conversations(
    ...
    service: Annotated[ResearchConversationUseCases, Depends(get_research_conversation_service)],
...
@router.get(
    ...
    responses={
        status.HTTP_200_OK: {
            "model": ResearchProgressEvent,
            "content": {
                "text/event-stream": {
                    "schema": {"$ref": "#/components/schemas/ResearchProgressEvent"}
                }
            },
        }
    },
...
async def stream_research_events(
    ...
    current_user: Annotated[UserAccount, Depends(get_current_user)],
    service: Annotated[ResearchConversationUseCases, Depends(get_research_conversation_service)],
    store: Annotated[ResearchEventStore, Depends(get_research_event_store)],
) -> StreamingResponse:
