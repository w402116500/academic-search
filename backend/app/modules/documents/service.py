...
class CandidateFulltextSubmission:
    ...
    search_run: CandidateFulltextRun
...
class CandidateFulltextService:
    ...
    def __init__(
        self,
        runs: CandidateFulltextRunPort,
        session_store: CandidateFulltextSessionStore,
    ...
    async def _load_candidate(
        ...
        *,
        owner_user_id: UUID,
        collection_id: UUID,
        run: CandidateFulltextRun,
        candidate_id: UUID,
    ) -> FulltextCandidate:
        """复用候选读取边界，并保留全文流程对外稳定的错误码。"""
        return await self._candidate_lookup.get(
            owner_user_id=owner_user_id,
            collection_id=collection_id,
            search_run_id=run.id,
            candidate_id=candidate_id,
    ...
    @staticmethod
    def _require_finished_search(run: CandidateFulltextRun) -> None:
