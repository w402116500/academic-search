 ...
 */
export function searchRunCandidateCount(candidateCounts: Record<string, unknown>): number {
  return (
    numericCount(candidateCounts, "candidate_count") ??
    numericCount(candidateCounts, "included_candidate_count") ??
    numericCount(candidateCounts, "included") ??
    numericCount(candidateCounts, "total") ??
    0
  );
}

/** 已通过相关性证据校验、真正进入用户筛选的候选数量。 */
export function searchRunScreeningCandidateCount(candidateCounts: Record<string, unknown>): number {
  return (
    numericCount(candidateCounts, "screening_candidate_count") ??
    searchRunCandidateCount(candidateCounts)
  );
}

/**
 * 相关性 Agent 会发布完整候选集合的总数、已处理数和已排除数。这里仅转换服务端的真实计数，
 * 不推算百分比或剩余时间，避免等待页向用户展示无法验证的进度。
 */
export function searchRunRelevanceProgress(candidateCounts: Record<string, unknown>): {
  total: number;
  analyzed: number;
  excluded: number;
} {
  return {
    total: numericCount(candidateCounts, "relevance_total_count") ?? 0,
    analyzed: numericCount(candidateCounts, "relevance_analyzed_count") ?? 0,
    excluded: numericCount(candidateCounts, "relevance_excluded_count") ?? 0,
  };
}

function numericCount(counts: Record<string, unknown>, key: string): number | undefined {
